...
- **Quiz feedback loop (2026-09-09):** siswa kini melihat penjelasan per soal, jawaban benar, dan indikator benar/salah (hijau/merah) setelah pengiriman kuis. `getAttemptFeedback` server action → `buildQuizFeedback()` fungsi TS murni → komponen `QuizResult`. Gate: `canShowScore()` menghormati kebijakan rilis guru (segera/nanti/draft).
- **Intervention queue workflow (2026-09-09):** tabel `alerts` diperpanjang dengan `assigned_to`, `due_at`, `follow_up_assessment_id`, `escalation_level`; status `'reopened'` ditambahkan. Tabel `notifications` baru untuk notifikasi in-app (RLS owner-only). Server actions: `assignAlert`, `reopenAlert`, `autoEscalateOverdue`, `createAlert`, `createNotification`, `markNotificationRead`. Student detail page menampilkan riwayat intervensi. NotificationBell terintegrasi di AppShell. 22 integration tests passing.
- **Screen reader a11y (2026-09-09):** `LiveAnnouncer` (polite+assertive aria-live regions), `useFocusTrap` hook (Tab trap + auto-focus + restore), `focusFirstError` helper. Fixed hardcoded Indonesian aria-labels in code-block, certificates. 5 new integration tests.
- **Load testing (2026-09-09):** k6 scripts for catalog browse, quiz flow, certificate verification. Config with thresholds (p95<500ms, error<1%). npm scripts: `load:catalog`, `load:quiz`, `load:cert`.
- **External integrations (2026-09-09):** `IntegrationAdapter` interface + adapter registry. Google Classroom adapter (OAuth2, roster sync, grade passback, token refresh). LTI 1.3 adapter (JWKS verification, NRPS, AGS). 10 integration tests.
- **Student analytics (2026-09-09):** `/analytics` page with summary cards (total minutes, avg score, attempts, courses), activity chart (14 days), quiz score trend, recent activity breakdown. Nav link added to student sidebar.
- **PWA (2026-09-09):** `manifest.json` (standalone, icons, theme), `sw.js` (network-first API, cache-first static, precache shell). 8 integration tests.
- **Caching layer (2026-09-09):** `lib/cache.ts` — in-memory stale-while-revalidate with TTL, background revalidation, `invalidateCache()`, `getCacheStats()`. 5 integration tests.
