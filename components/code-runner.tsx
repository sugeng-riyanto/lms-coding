...
export function CodeRunner({
  ...
  const def = useMemo(
    () => CODE_RUNNER_LANGUAGES.find((l) => l.id === lang) ?? CODE_RUNNER_LANGUAGES[0]!,
    [lang],
  );
  ...
  async function run() {
    ...
    try {
      ...
      try {
        ...
        body = { ok: false, error: "PROVIDER_ERROR", message: "Respons server tidak terbaca." };
      }
      if (res.status === 401) {
        body = { ok: false, error: "UNAUTHORIZED", message: "Silakan masuk dulu." };
      }
      setResult(body);
    } catch {
      setResult({
        ...
        message: "Gagal menghubungi server. Cek koneksi lalu coba lagi.",
      });
    } finally {
      setRunning(false);
    }
  ...
  return (
    <div className="overflow-hidden rounded-2xl border bg-white shadow-[var(--shadow-soft)] dark:bg-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b px-4 py-2.5">
        <p className="text-sm font-bold">Praktek coding — jalankan & lihat output</p>
        <button
          type="button"
          onClick={copyCode}
          disabled={!code}
      ...
      <div className="space-y-3 p-4">
        ...
        <details className="rounded-xl border border-dashed border-blue-300 bg-blue-50/60 px-3 py-2 dark:border-blue-800 dark:bg-blue-950/40">
          <summary className="cursor-pointer text-xs font-semibold text-blue-800 select-none dark:text-blue-300">
            🤖 Prompt AI — pahami/perbaiki kode ini
          </summary>
          <div className="mt-2 space-y-2">
            <p className="text-xs text-slate-600 dark:text-slate-300">
              Salin prompt lalu kirim ke AI bersama (opsional) output di atas. Prompt hanya memuat kode +
              output — tanpa data murid.
            </p>
