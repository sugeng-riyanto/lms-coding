...
describe("cspAlertState", () => {
  it("state awal tenang: tidak aktif, sampleSize 0", () => {
    const s = cspAlertState(T0);
  ...
  it("di bawah threshold: tidak spike, rate benar", () => {
    for (let i = 0; i < 10; i++) recordCspViolation(T0 + i);
    const s = cspAlertState(T0 + 10);
  ...
  it("rate ≥ threshold memicu spike", () => {
    for (let i = 0; i < 20; i++) recordCspViolation(T0 + i);
    expect(cspAlertState(T0 + 20).alertActive).toBe(true);
    // Satu lagi menaikkan rate.
    recordCspViolation(T0 + 21);
    expect(cspAlertState(T0 + 22).ratePerMin).toBe(21);
  });

  it("event lama keluar window → spike turun (decay)", () => {
    for (let i = 0; i < 30; i++) recordCspViolation(T0 + i);
    expect(cspAlertState(T0 + 30).alertActive).toBe(true);
    // Semua event berumur > window (61 detik kemudian).
    const s = cspAlertState(T0 + 30 + 61_000);
    expect(s.alertActive).toBe(false);
    expect(s.sampleSize).toBe(0);
  ...
  it("attempt yang di-block (rate-limiter) ikut dihitung untuk spike", () => {
    for (let i = 0; i < 19; i++) recordCspViolation(T0 + i);
    recordCspBlocked(T0 + 19);
    const s = cspAlertState(T0 + 20);
  ...
  it("threshold dari env dibaca & nilai buruk fallback ke 20", () => {
    process.env.CSP_ALERT_THRESHOLD_PER_MIN = "3";
    for (let i = 0; i < 3; i++) recordCspViolation(T0 + i);
    expect(cspAlertState(T0 + 3).alertActive).toBe(true);

    process.env.CSP_ALERT_THRESHOLD_PER_MIN = "abc";
    __resetCspAlerts();
    recordCspViolation(T0);
    expect(cspAlertState(T0).thresholdPerMin).toBe(20);
    expect(cspAlertState(T0).alertActive).toBe(false);
  });

  it("window 60 s membatasi sample (event lama keluar window)", () => {
